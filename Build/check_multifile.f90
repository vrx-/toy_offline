...
!
        ...
        ELSE
          IF (Master.and.Lcheck) THEN
            WRITE (stdout,10) 'Sources/Sinks DataQ', I_code
            DO ifile=1,Nfiles
              Tmin=Tscale*SSF(ng)%time_min(ifile)
              Tmax=Tscale*SSF(ng)%time_max(ifile)
              CALL time_string (Tmin, Tmin_code)
...
!
      DO i=1,nFfiles(ng)
        ...
        DO ifile=1,Nfiles
          ...
          foundit=check_file(ng, model, ncname, Tmin, Tmax, Tscale,     &
     &                       Lcheck)
          IF (FoundError(exit_flag, NoError, 309,                       &
     &                   "ROMS/Utility/check_multifile.F")) RETURN
...
!
      ...
      IF (FoundError(exit_flag, NoError, 576,                           &
...
!
     ...
     &                     ncid = ncid)
      IF (FoundError(exit_flag, NoError, 586,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
...
!
     ...
     &                     nVarDim = nvdim,                             &
     &                     nVarAtt = nvatt)
      IF (FoundError(exit_flag, NoError, 621,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
...
!
     ...
     &                      start = (/1/),                              &
     &                      total = (/1/))
      IF (FoundError(exit_flag, NoError, 667,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
      CALL netcdf_get_fvar (ng, model, ncname, TvarName,                &
     &                      Tmax,                                       &
     ...
     &                      start = (/Nrec/),                           &
     &                      total = (/1/))
      IF (FoundError(exit_flag, NoError, 675,                           &
     &               "ROMS/Utility/check_multifile.F")) RETURN
!
!  Close NetCDF file.
!
